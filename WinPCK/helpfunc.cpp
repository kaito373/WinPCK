...
BOOL TInstDlg::AddFiles()
{
	...
	if(IDCANCEL == MessageBoxW(L"Вы уверены, что хотите добавить файл?？", L" ", MB_OKCANCEL | MB_ICONQUESTION | MB_DEFBUTTON2))return FALSE;
...
void TInstDlg::AddSetupReg()
{
	...
	CreateAndSetDefaultValue(TEXT("pckfile\\shell\\open"), TEXT("Открыть с помощью WinPCK"));
	...
	CreateAndSetDefaultValue(TEXT("ZPWUpdatePack"), TEXT("Пакет обновления ZPW"));
	...
	CreateAndSetDefaultValue(TEXT("ZPWUpdatePack\\shell\\open"), TEXT("Открыть с помощью WinPCK"));
	CreateAndSetDefaultValue(TEXT("ZPWUpdatePack\\shell\\open\\command"), szStringExec);

	MessageBox(TEXT("Установка завершена."), TEXT("Информация"), MB_OK | MB_ICONASTERISK);
}
...
void TInstDlg::DeleteSetupReg()
{
	...
	MessageBox(TEXT("Удаление завершено."), TEXT("Информация"), MB_OK | MB_ICONASTERISK);
}
